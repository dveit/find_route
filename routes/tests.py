...
class AllTestCases(TestCase):
    ...
    def test_route_valid_form(self):
    ...
    def test_route_invalid_form(self):
        data = {'from_city': self.city_A.id,
                'to_city': self.city_B.id,
                'cities': [self.city_E.id, self.city_D.id],
                'traveling_time': 9.32
    ...
    def test_message_error_extra_time(self):
        data = {'from_city': self.city_A.id,
    ...
    def test_message_error_through_cities(self):
        ...
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Подходящих маршрутов нет', 1, 200)
